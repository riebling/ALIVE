...
namespace HttpServer.Templates
{
    ...
    public class SmartyEngine : ITemplateEngine
    {
        ...
        public string Render(string fileName, IDictionary<string, object> variables)
        {
            ...
            if (compiledTemplates.ContainsKey(fileName))
            {
                // Template exists, invoke it if has not been changed on disk
                TemplateInfo ti = compiledTemplates[fileName];
            ...
            return (string)o.GetType().InvokeMember("RunTemplate", BindingFlags.Instance | BindingFlags.Public | BindingFlags.InvokeMethod, null, o, new object[] { variables });
        }
        ...
        private object CompileTemplate(IList<string> assemblies, StringBuilder classText)
        {
