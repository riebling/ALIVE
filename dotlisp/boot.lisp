...
(def and (macro (&rest args)
	;MEH: relying on (rest nil) -> nil
	;(cond	(nil? args) true
	;		(nil? (rest args)) (first args)
	;		:else (list 'if (first args) (cons 'and (rest args)) nil))))
	(cond	(rest args) (list 'if (first args) (cons 'and (rest args)) nil)
			args (first args)
			:else true)))
...
(def (__backquote-expand x)
	(cond
		...
		(__starts-with? (first x) 'unquote-splicing)
			;MEH:nil? not required - rest is a list or nil
			;(if (nil? (rest x))
			;	(second (first x))
			(if (rest x)
...
(def (__pairize lst)
     (cond 
		(nil? lst)			nil
		;MEH:If len enumerates the whole list, this is O(n^2)
		;(odd? (len lst))	(error "Expecting even number of arguments")
		(nil? (rest lst))	(error "Expecting even number of arguments")
...
;MEH: BUGFIX: This was ~body.
(def-macro (letfn params &rest body)
	`(__letr ~(__pairize params) ~@body))

;MEH: BUGFIX: If there is no body, the last __set function was returned as a value. I inserted (block ...)
...
; MEH: (concat! ()) returns an error instead of (), so include when
(def (mapcat! f &rest lists)
	(let (lst (apply map->list f lists))
		(when lst
			(apply concat! lst))))
...
;MEH: To enable printing of enumerables, when required
(set *str-enum nil)
...
(def (concat &rest seqs)
	(make-enum
		(next-e (fn () (get-enum ;MEH first nil == nil: (if (nil? seqs)
					 ;				nil 
									(first seqs)));)
		 e  (next-e))
		(.IEnumerator:Current e)
...
(def (reduce f seq &key init)
	(lets (	e (get-enum seq)
			...
			result (cond		; MEH: () was (f), as document but nil seems more logical.
						(not has-items) (if (missing? init) () init)
...
; MEH: Explict version of (~key ~dictionary) avoids contradictions when key is a type for example
(def-macro (get-or-add dictionary key expr)
	(let (g (gensym))
		`(let (~g ;(~key ~dictionary)
                          (.IDictionary:get_Item ~dictionary ~key))
			(when (nil? ~g)
				(set ~g ~expr)
...
(def (__make-delegator-type type)
 (__make_type type "Delegator" "Invoke" MethodAttributes:Public)
)


(def (__make_type type type-suffix method-name method-attributes &key (interface false))
	(lets  (	invoke-sig (type.GetMethod method-name)
			invoke-arg-types (apply vector-of Type. 
								(map .ParameterType invoke-sig.GetParameters)) 
			tb (__delegator-module.DefineType (+ type.Name type-suffix)
					(bit-or TypeAttributes:Class TypeAttributes:Public) Object.)
			fn-field (tb.DefineField "fn" IFunction. FieldAttributes:Private)
			ctor-arg-types (if interface [IFunction. Object.] [IFunction.])
			cb (tb.DefineConstructor MethodAttributes:Public
								CallingConventions:Standard
								ctor-arg-types)
			mb (tb.DefineMethod method-name
			        method-attributes
					invoke-sig.ReturnType
					invoke-arg-types))
		(when interface
		 (tb.AddInterfaceImplementation type)
		 (tb.DefineMethodOverride mb invoke-sig)
		)
		(let (cil cb.GetILGenerator)
			...
			(when interface
				...
				(cil.Emit OpCodes:Stfld
					(tb.DefineField "Data" Object. FieldAttributes:Public)))
			(cil.Emit OpCodes:Ret))
		(let (mil mb.GetILGenerator)
			(mil.DeclareLocal (type-of []))
			;create an Object array the size of numargs + 1 for Me
			(mil.Emit OpCodes:Ldc_I4 (+ invoke-arg-types.Length (if interface 1 0)))
...
; Single method interface implementor:
(def (__make-interface-type type method-name)
 (__make_type type "Impl" method-name (bit-or MethodAttributes:Public MethodAttributes:Virtual) :interface true)
)
			
;get the interface implementation type for the interface if in cache, else create and cache one
(def (__make-interface type method-name f d)
	(let (interface-type (get-or-add __delegator-types type.FullName
							(__make-interface-type type method-name)))
		(interface-type f d)))

;make an instance of the delegate bound to the closure	
(def-macro (make-interface interface-method-name (&rest args) data &rest body)
	(when-not(is? interface-method-name CLSInstanceSymbol.)
		(error "interface-method-name syntax error: .TypeName:MethodName expected."))
	(lets (sym (interface-method-name.ToString)
		cp (sym.IndexOf ":")
		type (__typeize (sym.Substring 1 (- cp 1)))
